...
tf.app.flags.DEFINE_string('dataset_dir',
                           '/vol/medic02/users/wbai/data/cardiac_atlas/ACDC_2017/',
                           'Path to the dataset directory, which is split into training and validation '
                           'subdirectories.')
tf.app.flags.DEFINE_string('log_dir', '/vol/bitbucket/wbai/ukbb_cardiac/ACDC_2017/log',
...
def get_random_batch(filename_list, batch_size, image_size=192, data_augmentation=False,
    ...
    while n_selected < batch_size:
        ...
        if os.path.exists(image_name) and os.path.exists(label_name):
            ...
            label = (label == 3) * 1 + (label == 2) * 2 + (label == 1) * 3
...
def main(argv=None):
    ...
    data_list = {}
    for k in ['training_split_train', 'training_split_test']:
    ...
    with tf.Session() as sess:
        ...
        for iteration in range(1, 1 + FLAGS.train_iteration):
            ...
            images, labels = get_random_batch(data_list['training_split_train'],
            ...
            if iteration % 10 == 0:
                ...
                images, labels = get_random_batch(data_list['training_split_test'],
